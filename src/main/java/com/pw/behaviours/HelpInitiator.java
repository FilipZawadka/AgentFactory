...
public class HelpInitiator extends ContractNetInitiator {
	private final int trNumber;
	private int trNumberTmp;
	...
	protected void handleAllResponses(Vector responses, Vector acceptances) {
		...
		if (ce instanceof Action && (((Action) ce).getAction()) instanceof GetHelp) {
			...
			if (this.trNumberTmp > 0)
				myAgent.addBehaviour(new StartJobBehaviour(myAgent, conversation_id, destination,
						(ACLMessage) (getDataStore().get(CFP_KEY))));
			else {
				System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! CONTRACT BROKEN");
				myAgent.addBehaviour(
						new HelpInitiator(myAgent, (ACLMessage) getDataStore().get(CFP_KEY), this.trNumber));
			}
		}
	}
