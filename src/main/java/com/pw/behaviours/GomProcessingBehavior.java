...
public class GomProcessingBehavior extends TickerBehaviour {
	...
	private boolean isThereEnoughMaterialsToStartProcess(Map<Material, Integer> availableMaterials,
			Map<Material, Integer> requiredMaterials) {
		for (Map.Entry<Material, Integer> entry : requiredMaterials.entrySet()) {
			if (!availableMaterials.containsKey(entry.getKey()) || availableMaterials.get(entry.getKey()) < entry
					.getValue()) {
				return false;
			}
		}
